/*--------------------------------------------------------------------
 *	$Id: gmt_calclock.c,v 1.1 2008/08/13 15:59:46 tgray Exp $
 *
...
GMT_LONG	splitinteger(double value, int epsilon, double *doublepart) {
...
*/
	return ((GMT_cal_rd)(date - GMT_cal_imod((int)(date-kday), 7)));
}
...
/*	Given rata die integer day number, load calendar structure
	...
	gcal->day_w = GMT_cal_imod ((int)date, 7);
...
{	/* Assemble the annotation label given the formatting options presented */
	...
	switch (T->unit) {
		...
		case 'c':	/* 2-digit seconds */
			(P->date.compact) ? sprintf (string, "%d", (int)irint(calendar.sec)) : sprintf (string, "%2.2d", (int)irint(calendar.sec));
			break;
		default:
