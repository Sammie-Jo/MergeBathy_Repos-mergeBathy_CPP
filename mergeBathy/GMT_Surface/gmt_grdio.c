/*--------------------------------------------------------------------
 *	$Id: gmt_grdio.c,v 1.1 2008/08/13 16:02:38 tgray Exp $
 *
...
int GMT_write_grd (char *file, struct GRD_HEADER *header, float *grid, double w, double e, double s, double n, int *pad, BOOLEAN complex, NV_FLOAT64 *z)
{	/* file:	File name
	 * header:	grid structure header
	 * grid:	array with final grid
	...
	return ((*GMT_io_writegrd[header->type]) (header, grid, w, e, s, n, pad, complex, z));
}
...
int GMT_grd_prep_io (struct GRD_HEADER *header, double *w, double *e, double *s, double *n, GMT_LONG *width, GMT_LONG *height, GMT_LONG *first_col, GMT_LONG *last_col, GMT_LONG *first_row, GMT_LONG *last_row, int **index)
{
	...
	k = (int *) GMT_memory (VNULL, (size_t)(*width), sizeof (int), "GMT_grd_prep_io");
	if (geo) {
		for (i = 0; i < (*width); i++) {
